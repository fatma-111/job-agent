...
/* The rule above forces light text everywhere in the sidebar for readability
   against the dark background — but skill tags carry their own readable
   text/background pairing (light green bg + dark green text) and must not
   be flattened to invisible light-on-light text by that blanket rule. */
section[data-testid="stSidebar"] .tag.have {
  color:#046C4E !important; background:#E6F6EF !important; border-color:#B7E4CE !important;
}
section[data-testid="stSidebar"] .tag.gap {
  color:#93370D !important; background:#FEF3E7 !important; border-color:#F5D6B3 !important;
}
