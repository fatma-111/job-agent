...
railway init                   # creates a new Railway project, uses railway.json
railway variables set OPENROUTER_API_KEY=sk-or-v1-...
railway variables set SMTP_HOST=smtp.gmail.com SMTP_PORT=587 \
...
Notes:
- Railway assigns `$PORT` dynamically for the one public-facing process; the
  entrypoint script maps that to Streamlit and runs the FastAPI backend on an
  internal port, avoiding collisions automatically.
- Build takes a few minutes the first time — it installs Chromium and
  pre-downloads the embedding model into the image so cold starts are fast.
- **Add a persistent volume** or the SQLite database resets on every deploy:
  Railway dashboard → your service → **Settings → Volumes** → mount path
  `/app/data`. Without this, CVs/applications/alerts vanish on redeploy.
- If the health check times out on first deploy, check `railway logs` —
  it's almost always either a missing env var or the build still running.
